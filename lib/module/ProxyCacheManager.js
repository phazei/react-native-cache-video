import { HLS_VIDEO_TYPE, KEY_PREFIX, SIGNAL_NOT_DOWNLOAD_ACTION
// VIDEO_EXTENSIONS,
} from './Utils/constants';
import { FileBucket, FileSystemManager } from './Libs/fileSystem';
import { SimpleSessionProvider } from './Libs/session';
import { absoluteFilePath, cacheKey, getCacheKey, getOriginURL, isHLSUrl, reverseProxyPlaylist, reverseProxyURL } from './Utils/util';
...
export class CacheManager {
  ...
  //
  constructor(serverName, devMode, _sessionTask = new SimpleSessionProvider(), _storage = new FileSystemManager()) {
    //
  ...
  putCachedFile(forKey, folder) {
    ...
    } = getCacheKey(forKey, folder, KEY_PREFIX);
  ...
  async getCachedFileAsync(url, folder = this.cacheFolder) {
    ...
    const {
      originURL,
      cacheKey: cacheKeyStr
    } = getCacheKey(url, folder, KEY_PREFIX);
  ...
  enableMemoryCache(cachePolicy) {
    if (!this._memoryCache) {
      this._memoryCache = new MemoryCacheProvider(cachePolicy);
  ...
  async removeCachedVideo(url) {
    ...
    const {
      originURL
    } = getCacheKey(url, this.cacheFolder, KEY_PREFIX);
    const key = originURL.href;
  ...
  async onCachingPlaylistSource(forUrl, data, folder) {
    const {
      originURL,
      cacheKey: cacheKeyStr
    } = getCacheKey(forUrl, folder, KEY_PREFIX);
