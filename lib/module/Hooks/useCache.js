...
export const useAsyncCache = () => {
  const currentVideoUrl = useRef(undefined);
  const [cachedVideoUrl, setVideoUrl] = useState(undefined);
  //
  const {
    cacheManager
  } = useProxyCacheManager();
  const delayUpdateVideo = useCallback(videoFile => setTimeout(() => {
  ...
  const setVideoPlayUrlBy = useCallback(async newUrl => {
    // in case onLayout call multiple times
    if (newUrl && cacheManager) {
      const isStream = isHLSUrl(newUrl);
  ...
  const removeVideoFromCache = useCallback(async url => {
    if (cacheManager) {
