...
export class LFUSizePolicy {
  ...
  onAccess(cache, key) {
    ...
    this.referenceBit[key] = isNull(this.referenceBit[key]) ? SECOND_CHANCE_TO_COUNT : this.referenceBit[key] + 1;
  }
  async onEvict(cache, delegate, triggerKey) {
    ...
    try {
      ...
      const files = await this.storage.getStatisticList(this.storage.getBucketFolder(FileBucket.cache));
