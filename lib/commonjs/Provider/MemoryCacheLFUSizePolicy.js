...
class LFUSizePolicy {
  isEvicting = false;
  constructor(capacityMB) {
    this.referenceBit = {};
    this.capacityBytes = capacityMB * 1024 * 1024; // Convert MB to bytes
    this.storage = new _fileSystem.FileSystemManager();
  }
  clear() {
    this.referenceBit = {};
  }
  ...
  onAccess(cache, key) {
    ...
    this.referenceBit[key] = (0, _util.isNull)(this.referenceBit[key]) ? _constants.SECOND_CHANCE_TO_COUNT : this.referenceBit[key] + 1;
  }
  async onEvict(cache, delegate, triggerKey) {
    if (this.isEvicting) {
    ...
    try {
      ...
      const files = await this.storage.getStatisticList(this.storage.getBucketFolder(_fileSystem.FileBucket.cache));
      let totalSize = files.reduce((sum, file) => sum + parseInt(file.size, 10), 0);
  ...
  findLFUKey(files, cache, excludeKey) {
    ...
    for (const key in this.referenceBit) {
      ...
      if (freq && freq < minFreq) {
        if (freq !== _constants.SECOND_CHANCE_TO_COUNT || lfuKey === null) {
          minFreq = freq;
          lfuKey = key;
        }
      }
    ...
    if (!lfuKey && Object.keys(this.referenceBit).length > 0) {
      ...
      lfuKey = Array.from(cache.entries()).find(([_, path]) => path.includes(oldestFile.filename))?.[0] || cache.keys().next().value ||
      // fallback to first (oldest) key
      null;
    }
    return lfuKey;
  }
  ...
  set dataSource(data) {
    const newDataSource = (0, _util.mergeWithCustomCondition)(this.referenceBit, data, _util.mergeLargerNumber);
    this.referenceBit = newDataSource;
  }
}
exports.LFUSizePolicy = LFUSizePolicy;
//# sourceMappingURL=MemoryCacheLFUSizePolicy.js.map